(function($) {
	...
	if ($('.services-carousel-two').length) {
		$('.services-carousel-two').owlCarousel({
			items: 3, // Corrigido de 'intems' para 'items'
			margin: 30,
			loop: true,
			nav: true,
			smartSpeed: 700,
...
$(document).ready(function() {
    ...
    $whatsappButton.hide(); // Ocultar ao carregar

    $(window).on('scroll', function() {
        if ($(this).scrollTop() > 200) {
            $whatsappButton.fadeIn(); // Aparecer ao rolar
        } else {
            $whatsappButton.fadeOut(); // Desaparecer se não estiver rolando
        }
    });
});
